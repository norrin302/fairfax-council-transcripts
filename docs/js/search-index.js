...
// Supports multiple meetings with full-text search
...
const SEARCH_INDEX = {
  ...
  meetings: [
    {
      ...
      duration_seconds: 600, // 10 min excerpt
      full_duration_seconds: 10020, // 2h 47m
  ...
  // Transcript segments indexed for search
  segments: [
    {
      meeting_id: "apr-14-2026",
      meeting_date: "2026-04-14",
  ...
  // Unique speakers for filter dropdown (computed from segments)
  get speakers() {
    return [...new Set(this.segments.map(s => s.speaker))];
  },
  
  // Get meeting by ID
  getMeeting: function(meetingId) {
    return this.meetings.find(m => m.meeting_id === meetingId);
  },
  
  // Get segments for a specific meeting
  getMeetingSegments: function(meetingId) {
    return this.segments.filter(s => s.meeting_id === meetingId);
  },
  
  // Add a new meeting (call before adding segments)
  addMeeting: function(meeting) {
    this.meetings.push(meeting);
  },
  
  // Add segments for a meeting
  addSegments: function(newSegments) {
